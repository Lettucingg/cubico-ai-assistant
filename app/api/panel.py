...
def _anonimizar_texto_exportado(texto: str, nombres: list[str] | None = None) -> str:
    ...
    for nombre in sorted(
        {n.strip() for n in nombres or [] if n and n.strip()},
        key=len,
        reverse=True,
    ):
        resultado = re.sub(
...
def _crear_exportacion_conversacion(
    historial: list[dict],
    ...
    for mensaje in historial:
        ...
        error = _anonimizar_texto_exportado(
            mensaje.get("error_entrega") or "",
            nombres,
        )
        ...
        lineas.append(
            f"[{_fecha_exportada(mensaje.get('timestamp'))}] "
            f"{rol}: {contenido}{detalle}"
        )
    return "\n".join(lineas) + "\n"
...
def exportar_conversacion(
    ...
    if sesion.codigo_cliente_verificado:
        try:
            ...
            if info.get("encontrado") and info.get("nombre_completo"):
                ...
                nombres.extend(
                    parte for parte in nombre_completo.split() if len(parte) >= 4
                )
        except Exception:
            # La descarga debe seguir disponible aunque falle la consulta
            # temporal de datos del cliente.
            pass

    contenido = _crear_exportacion_conversacion(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="conversacion-cubico-{fecha}.txt"'
            ),
